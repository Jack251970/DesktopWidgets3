...
public class ShellStorageFolder : BaseStorageFolder
{
    ...
    public static ShellStorageFolder FromShellItem(ShellFileItem item)
    {
        ...
        return item.RecyclePath!.Contains("$Recycle.Bin", StringComparison.OrdinalIgnoreCase)
            ? new BinStorageFolder(item)
            : new ShellStorageFolder(item);
    }
    ...
    public static IAsyncOperation<BaseStorageFolder> FromPathAsync(string path)
    {
        return AsyncInfo.Run<BaseStorageFolder>(async (cancellationToken) =>
        {
            if (IsShellPath(path))
            {
                var (Folder, Items) = await GetFolderAndItems(path, false);
                if (Folder is not null)
                {
                    return FromShellItem(Folder);
                }
            }
            return null!;
        });
    ...
    public override IAsyncOperation<BaseBasicProperties> GetBasicPropertiesAsync()
    {
        ...
        {
            var (Folder, Items) = await GetFolderAndItems(Path, false);
            return Folder is not null ? new ShellFolderBasicProperties(Folder) : new BaseBasicProperties();
        });
    }
    ...
    public override IAsyncOperation<IStorageItem> GetItemAsync(string name)
    {
        ...
        {
            var (Folder, Items) = await GetFolderAndItems(Path, true);
            if (Items is null)
            {
                return null!;
            }

            var entry = Items.FirstOrDefault(x => x.FileName is not null && x.FileName.Equals(name, StringComparison.OrdinalIgnoreCase));
            if (entry is null)
            {
                return null!;
            }
    ...
    public override IAsyncOperation<IReadOnlyList<IStorageItem>> GetItemsAsync(uint startIndex, uint maxItemsToRetrieve)
    {
        ...
        {
            var (Folder, Items) = await GetFolderAndItems(Path, true, (int)startIndex, (int)maxItemsToRetrieve);
            if (Items is null)
            {
                return null!;
            }
