...
public sealed class SingleFileDatabaseModel(string fileName, IModifiableFolder settingsFolder, IAsyncSerializer<Stream> serializer) : BaseDatabaseModel<ISerializedModel>(serializer)
{
	private readonly string _fileName = fileName;
	private readonly IModifiableFolder _settingsFolder = settingsFolder;
	...
	/// <inheritdoc/>
	public async override Task LoadAsync(CancellationToken cancellationToken = default)
	{
		try
		{
	...
	/// <inheritdoc/>
	public async override Task<bool> SaveAsync(CancellationToken cancellationToken = default)
	{
		try
		{
			await storageSemaphore.WaitAsync(cancellationToken);
	...
	private sealed class NonSerializedData(object? value) : ISerializedModel
	{
