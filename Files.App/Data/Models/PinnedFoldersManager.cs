	...
	public async Task<LocationItem> CreateLocationItemFromPathAsync(string path)
	{
		...
		if (res)
		{
			...
			if (res && res.Result is not null)
			{
				...
				var bitmapImage = await ThreadExtensions.MainDispatcherQueue.EnqueueOrInvokeAsync(() => locationItem.IconData.ToBitmapAsync(), Microsoft.UI.Dispatching.DispatcherQueuePriority.Normal);
		...
		{
			locationItem.Icon = (await ThreadExtensions.MainDispatcherQueue.EnqueueOrInvokeAsync(() => UIHelpers.GetSidebarIconResource(Constants.ImageRes.Folder)))!;
