...
public sealed class ShellViewModel : ObservableObject, IDisposable
{
    ...
    private readonly INetworkService NetworkService = DependencyExtensions.GetRequiredService<INetworkService>();
	...
	public ShellViewModel(IFolderViewViewModel folderViewViewModel, LayoutPreferencesManager folderSettingsViewModel)
	{
    ...
    private static bool CheckElevationRights(ListedItem item)
    {
	...
	private async Task<int> EnumerateItemsFromStandardFolderAsync(string path, CancellationToken cancellationToken, LibraryItem? library = null)
	{
		...
		if (isNetwork)
		{
            var auth = await NetworkService.AuthenticateNetworkShare(FolderViewViewModel, path);
            if (!auth)
            {
                return -1;
            }
	...
	private void WatchForDirectoryChanges(string path, CloudDriveSyncStatus syncStatus)
	{
        ...
        var hWatchDir = Win32PInvoke.CreateFileFromApp(path, 1, 1 | 2 | 4,
            IntPtr.Zero, 3, (uint)Win32PInvoke.File_Attributes.BackupSemantics | (uint)File_Attributes.Overlapped, IntPtr.Zero);
