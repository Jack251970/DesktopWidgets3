...
public sealed class AddressToolbarViewModel : ObservableObject, IAddressToolbarViewModel, IDisposable
{
	...
	private Style _LayoutOpacityIcon = null!;
	...
	private string pathControlDisplayText = null!;
	...
	private AddressToolbar? AddressToolbar => (FolderViewViewModel.Content as Frame)?.FindDescendant<AddressToolbar>();
	...
	public bool CanCopy => SelectedItems is not null && SelectedItems.Count != 0;
	public bool CanExtract => IsArchiveOpened ? (SelectedItems is null || SelectedItems.Count == 0) : IsSelectionArchivesOnly;
	public bool IsArchiveOpened => FileExtensionHelpers.IsZipFile(Path.GetExtension(pathControlDisplayText));
	public bool IsSelectionArchivesOnly => SelectedItems is not null && SelectedItems.Count != 0 && SelectedItems.All(x => FileExtensionHelpers.IsZipFile(x.FileExtension)) && !InstanceViewModel.IsPageTypeRecycleBin;
	...
	public bool IsImage => SelectedItems is not null && SelectedItems.Count != 0 && SelectedItems.All(x => FileExtensionHelpers.IsImageFile(x.FileExtension)) && !InstanceViewModel.IsPageTypeRecycleBin;
	...
	public bool IsFont => SelectedItems is not null && SelectedItems.Count != 0 && SelectedItems.All(x => FileExtensionHelpers.IsFontFile(x.FileExtension)) && !InstanceViewModel.IsPageTypeRecycleBin;

    // CHANGE: DetailsLayout if FolderSettings is null.
	...
	public bool IsGridLayout => instanceViewModel.FolderSettings is not null && instanceViewModel.FolderSettings.LayoutMode is FolderLayoutModes.GridView;
	public bool IsDetailsLayout => instanceViewModel.FolderSettings is null || instanceViewModel.FolderSettings.LayoutMode is FolderLayoutModes.DetailsView;
	public bool IsListLayout => instanceViewModel.FolderSettings is not null && instanceViewModel.FolderSettings.LayoutMode is FolderLayoutModes.ListView;

    // CHANGE: False if FolderSettings is null.
    public bool IsLayoutSizeCompact =>
        UserSettingsService is not null &&
    ...
    // CHANGE: True if FolderSettings is null.
    public bool IsLayoutSizeSmall =>
        UserSettingsService is null ||
