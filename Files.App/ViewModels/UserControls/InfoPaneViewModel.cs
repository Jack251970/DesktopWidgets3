...
public class InfoPaneViewModel : ObservableObject, IDisposable
{
	...
	private UIElement previewPaneContent = null!;
	...
	private void ContentPageContext_PropertyChanged(object? sender, PropertyChangedEventArgs e)
	{
		switch (e.PropertyName)
		{
			...
			case nameof(IContentPageContext.SelectedItem):
                ...
                var shouldUpdatePreview = ((FolderViewViewModel.MainWindow.Content as Frame)?.Content as MainPage)?.ShouldPreviewPaneBeActive;
	...
	private async Task<UserControl> GetBuiltInPreviewControlAsync(ListedItem item, bool downloadItem)
	{
		...
		)
		{
			...
			return new ShellPreview(FolderViewViewModel, model);
		}
		...
		if (SelectedItem is not null && ContentPageContext.SelectedItems.Count == 1)
		{
			...
			try
			{
					...
					SelectedItem?.PrimaryItemAttribute == StorageItemTypes.Folder)
				{
					...
					await LoadPreviewControlAsync(downloadItem, loadCancellationTokenSource.Token);
				}
				else
				{
