...
public sealed class SidebarViewModel : ObservableObject, IDisposable, ISidebarViewModel
{
    ...
    private IShellPanesPage paneHolder = null!;
    ...
    public void Initialize(IFolderViewViewModel folderViewViewModel)
    {
        ...
        RestoreLibrariesCommand = new AsyncRelayCommand(() => LibraryManager.ShowRestoreDefaultLibrariesDialogAsync(FolderViewViewModel));
    }
    ...
    private static async Task SyncSidebarItemsAsync(LocationItem section, Func<IReadOnlyList<INavigationControlItem>> getElements, NotifyCollectionChangedEventArgs e)
    {
        if (section is null)
        {
            return;
    ...
    private static async Task AddElementToSectionAsync(INavigationControlItem elem, LocationItem section, int index = -1)
    {
    ...
    private async void ItemContextMenuFlyout_Opened(object? sender, object e)
    {
        ...
        itemContextMenuFlyout.Opened -= ItemContextMenuFlyout_Opened;
        await ShellContextFlyoutFactory.LoadShellMenuItemsAsync(FolderViewViewModel, rightClickedItem.Path, itemContextMenuFlyout, rightClickedItem.MenuOptions);
    }
    ...
    public async void HandleItemInvokedAsync(object item, PointerUpdateKind pointerUpdateKind)
    {
        ...
        {
            await NavigationHelpers.OpenPathInNewTab(FolderViewViewModel, navigationControlItem.Path, false);
            return;
        }
    ...
    public ICommand CreateLibraryCommand { get; private set; } = null!;

    public ICommand RestoreLibrariesCommand { get; private set; } = null!;
    ...
    private async Task OpenInNewTabAsync()
    {
        ...
        await NavigationHelpers.OpenPathInNewTab(FolderViewViewModel, rightClickedItem.Path, false);
    }
    ...
    private void PinItem()
    {
        ...
        {
            _ = QuickAccessService.PinToSidebarAsync([rightClickedItem.Path]);
        }
    }
    ...
    private static async Task HandleLocationItemDragOverAsync(LocationItem locationItem, ItemDragOverEventArgs args)
    {
    ...
    private static async Task HandleDriveItemDragOverAsync(DriveItem driveItem, ItemDragOverEventArgs args)
    {
        if (!Utils.Storage.FilesystemHelpers.HasDraggedStorageItems(args.DroppedItem))
        {
            return;
    ...
    private static async Task HandleTagItemDragOverAsync(FileTagItem tagItem, ItemDragOverEventArgs args)
    {
        if (!Utils.Storage.FilesystemHelpers.HasDraggedStorageItems(args.DroppedItem))
        {
            return;
