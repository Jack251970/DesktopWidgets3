...
public sealed class NetworkLocationsWidgetViewModel : BaseWidgetViewModel, IWidgetViewModel
{
	...
	{
		if (await DriveHelpers.CheckEmptyDrive(FolderViewViewModel, path))
        {
            return;
        }
	...
	{
		if (item is null || await DriveHelpers.CheckEmptyDrive(FolderViewViewModel, item.Item.Path))
        {
            return;
        }
	...
	{
		return NetworkService.OpenMapNetworkDriveDialogAsync(FolderViewViewModel);
	}
	...
	private void ExecuteOpenPropertiesCommand(WidgetDriveCardItem? item)
	{
		...
		flyoutClosed = (s, e) =>
		{
			...
			FilePropertiesHelpers.OpenPropertiesWindow(FolderViewViewModel, item.Item, ContentPageContext.ShellPage!);
		};
	...
	private async Task UpdateItems(ObservableCollection<ILocatableFolder> source)
	{
		await ThreadExtensions.MainDispatcherQueue.EnqueueOrInvokeAsync(async () =>
		{
			...
			foreach (var drive in source.ToList().Cast<DriveItem>())
			{
			...
			foreach (var driveCard in Items.ToList())
			{
