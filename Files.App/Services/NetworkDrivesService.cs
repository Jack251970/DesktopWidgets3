...
{
    private static readonly string guid = "::{f02c1a0d-be21-4350-88b0-7367fc96ef3c}";
    ...
    {
        var (Folder, Enumerate) = await Win32Helper.GetShellFolderAsync(guid, false, true, 0, int.MaxValue);

        return Enumerate.Where(item => item.IsFolder).Select(item =>
        {
            var networkItem = new DriveItem()
            {
                Text = item.FileName,
    ...
    public async Task UpdateDrivesAsync()
    {
        ...
        foreach (var item in await GetDrivesAsync())
        {
    ...
    /// <inheritdoc/>
    public Task OpenMapNetworkDriveDialogAsync(IFolderViewViewModel folderViewViewModel)
    {
        var hWnd = folderViewViewModel.WindowHandle.ToInt64();
    ...
    /// <inheritdoc/>
    public async Task<bool> AuthenticateNetworkShare(IFolderViewViewModel folderViewViewModel, string path)
    {
        ...
        var connectionError = WNetAddConnection3(HWND.NULL, netRes, null, null, 0);

        if (connectionError == Win32Error.ERROR_LOGON_FAILURE || connectionError == Win32Error.ERROR_ACCESS_DENIED)
        {
            var dialog = DynamicDialogFactory.GetFor_CredentialEntryDialog(folderViewViewModel, path);
            ...
            if (credentialsReturned is not null && credentialsReturned[1] != null)
            {
                ...
                if (credentialsReturned[2] == "y" && connectionError == Win32Error.NO_ERROR)
                {
                    ...
                    var bPassword = Encoding.Unicode.GetBytes(credentialsReturned[1]);
        ...
        {
            await DialogDisplayHelper.ShowDialogAsync(folderViewViewModel, "NetworkFolderErrorDialogTitle".GetLocalizedResource(), connectionError.ToString().Split(":")[1].Trim());
