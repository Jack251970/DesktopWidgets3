...
internal sealed class FileExplorerService : IFileExplorerService
{
	...
	/// <inheritdoc/>
	public async Task<ILocatableFile?> PickSingleFileAsync(IFolderViewViewModel viewModel, IEnumerable<string>? filter, CancellationToken cancellationToken = default)
	{
		var filePicker = InitializeWithWindow(viewModel, new FileOpenPicker());
	...
	/// <inheritdoc/>
	public async Task<ILocatableFolder?> PickSingleFolderAsync(IFolderViewViewModel viewModel, CancellationToken cancellationToken = default)
	{
		var folderPicker = InitializeWithWindow(viewModel, new FolderPicker());
