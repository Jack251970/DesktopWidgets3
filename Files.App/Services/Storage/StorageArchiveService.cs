...
{
	/*private StatusCenterViewModel StatusCenterViewModel { get; } = DependencyExtensions.GetRequiredService<StatusCenterViewModel>();*/
	private IThreadingService ThreadingService { get; } = DependencyExtensions.GetRequiredService<IThreadingService>();
	...
	/// <inheritdoc/>
	public async Task<bool> CompressAsync(IFolderViewViewModel folderViewViewModel, ICompressArchiveModel compressionModel)
	{
		...
		var isSuccess = await compressionModel.RunCreationAsync();
	...
	/// <inheritdoc/>
	public async Task<bool> DecompressAsync(IFolderViewViewModel folderViewViewModel, string archiveFilePath, string destinationFolderPath, string password = "")
	{
	...
	{
		using var zipFile = await GetSevenZipExtractorAsync(archiveFilePath);
		if (zipFile is null)
        {
            return true;
        }
	...
	public async Task<SevenZipExtractor?> GetSevenZipExtractorAsync(string archiveFilePath, string password = "")
	{
		...
		{
			var archive = await StorageHelpers.ToStorageItem<BaseStorageFile>(archiveFilePath);
