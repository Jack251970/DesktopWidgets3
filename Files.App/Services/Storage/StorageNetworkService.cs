...
public sealed class NetworkService : ObservableObject, INetworkService
{
    ...
    public Task OpenMapNetworkDriveDialogAsync(IFolderViewViewModel folderViewViewModel)
    {
        return Win32Helper.StartSTATask(() =>
        {
            return CommonDialogService.Open_NetworkConnectionDialog(
                folderViewViewModel.WindowHandle,
    ...
    public async Task<bool> AuthenticateNetworkShare(IFolderViewViewModel folderViewViewModel, string path)
    {
        ...
        if (res == WIN32_ERROR.ERROR_LOGON_FAILURE || res == WIN32_ERROR.ERROR_ACCESS_DENIED)
        {
            ...
            if (credentialsReturned is not null && credentialsReturned[1] != null)
            {
                ...
                if (credentialsReturned[2] == "y" && res == WIN32_ERROR.NO_ERROR)
                {
                    ...
                    unsafe
                    {
                        fixed (char* lpcTargetName = path[2..])
                        {
        ...
        {
            await DialogDisplayHelper.ShowDialogAsync(folderViewViewModel, "NetworkFolderErrorDialogTitle".GetLocalizedResource(), res.ToString());
