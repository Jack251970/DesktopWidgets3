...
public class NativeFileOperationsHelper
{
    ...
    private class WIN32_FIND_STREAM_DATA
    {
        ...
        public string cStreamName = null!;
    }
	...
	{
		var hFile = CreateFileFromApp(filePath,
			GENERIC_READ,
			FILE_SHARE_READ,
			IntPtr.Zero,
			OPEN_EXISTING,
	...
	{
		var hStream = CreateFileFromApp(filePath,
	...
	public static bool WriteBufferToFileWithProgress(string filePath, byte[] buffer, LPOVERLAPPED_COMPLETION_ROUTINE callback)
	{
		...
		var result = WriteFileEx(hFile.DangerousGetHandle(), buffer, (uint)buffer.LongLength, ref nativeOverlapped, callback);
	...
	public static ulong? GetFolderFRN(string folderPath)
	{
		...
		if (!handle.IsInvalid)
		{
			var _ = new FILE_ID_BOTH_DIR_INFO();
			if (GetFileInformationByHandleEx(handle.DangerousGetHandle(), FILE_INFO_BY_HANDLE_CLASS.FileIdBothDirectoryInfo, out _, (uint)Marshal.SizeOf(_)))
			{
				return (ulong)_.FileId;
			}
		}
		return null;
	}
    ...
    public static IEnumerable<(string Name, long Size)> GetAlternateStreams(string path)
    {
        ...
        var hFile = FindFirstStreamW(path, StreamInfoLevels.FindStreamInfoStandard, findStreamData, 0);
