...
public static partial class Win32Helper
{
	...
	public static string?[] CommandLineToArgs(string commandLine)
	{
        ...
        var argv = Shell32.CommandLineToArgvW(commandLine, out var argc);
		if (argv == IntPtr.Zero)
        {
	...
	private static readonly object _iconOverlayLock = new();
	...
	public static byte[]? GetIconOverlay(string path, bool isDirectory)
	{
		...
		try
		{
			var result = Shell32.SHGetFileInfo(path, isDirectory ? FileAttributes.Directory : 0, ref shFileInfo, Shell32.SHFILEINFO.Size, flags);
	...
	public static IList<IconFileInfo>? ExtractIconsFromDLL(string file)
	{
        ...
        var count = icoCnt.DangerousGetHandle().ToInt32();
		if (count <= 0)
        {
            return null;
        }
	...
	{
		var hFile = Win32PInvoke.CreateFileFromApp(filePath,
	...
	{
		var hStream = Win32PInvoke.CreateFileFromApp(filePath,
	...
	public static bool WriteBufferToFileWithProgress(string filePath, byte[] buffer, Win32PInvoke.LPOVERLAPPED_COMPLETION_ROUTINE callback)
	{
		...
		var result = Win32PInvoke.WriteFileEx(hFile.DangerousGetHandle(), buffer, (uint)buffer.LongLength, ref nativeOverlapped, callback);
	...
	public static ulong? GetFolderFRN(string folderPath)
	{
		...
		if (!handle.IsInvalid)
		{
            ...
            if (Win32PInvoke.GetFileInformationByHandleEx(handle.DangerousGetHandle(), Win32PInvoke.FILE_INFO_BY_HANDLE_CLASS.FileIdBothDirectoryInfo, out var fileStruct1, (uint)Marshal.SizeOf(fileStruct)))
			{
				return (ulong)fileStruct1.FileId;
			}
		}
		return null;
	}
	...
	public static string ParseSymLink(string path)
	{
		...
		if (!handle.IsInvalid)
		{
			if (Win32PInvoke.DeviceIoControl(handle.DangerousGetHandle(), Win32PInvoke.FSCTL_GET_REPARSE_POINT, IntPtr.Zero, 0, out var buffer, Win32PInvoke.MAXIMUM_REPARSE_DATA_BUFFER_SIZE, out _, IntPtr.Zero))
			{
				...
				if (string.IsNullOrEmpty(normalisedTarget))
				{
					...
					{
						normalisedTarget = normalisedTarget[4..];
					}
				}
				if (buffer.ReparseTag == Win32PInvoke.IO_REPARSE_TAG_SYMLINK && (normalisedTarget.Length < 2 || normalisedTarget[1] != ':'))
				{
					...
					normalisedTarget = Path.GetFullPath(Path.Combine(path[..path.LastIndexOf(Path.DirectorySeparatorChar)], normalisedTarget));
				}
	...
	public static IEnumerable<(string Name, long Size)> GetAlternateStreams(string path)
	{
		...
		var hFile = Win32PInvoke.FindFirstStreamW(path, Win32PInvoke.StreamInfoLevels.FindStreamInfoStandard, findStreamData, 0);
