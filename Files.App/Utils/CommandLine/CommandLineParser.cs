...
public sealed class CommandLineParser
{
	...
	{
		var commandLineCharArray = commandLine.ToCharArray();
		var isInQuote = false;

		for (var i = 0; i < commandLineCharArray.Length; i++)
		{
			...
			if (args.Length > 2)
			{
				for (var i = 0; i < args.Length; i++)
				{
					if (args[i].StartsWith('-') || args[i].StartsWith('/'))
					{
						...
						{
							for (var j = 0; j < parsedArgs.Count; j++)
							{
	...
	private static KeyValuePair<string, string[]> ParseData(string[] args, int index)
	{
		...
		if (args[index].StartsWith('-') || args[index].StartsWith('/'))
		{
			if (args[index].Contains(':', StringComparison.Ordinal))
			{
				...
				var endIndex = argument.IndexOf(':');

				// Trim the '/' and the ':'
				key = argument[1..endIndex];

				var valueStart = endIndex + 1;
