...
public static partial class GitHelpers
{
	...
	public static async Task<bool> Checkout(string? repositoryPath, string? branch)
	{
		...
		var result = await PostMethodToThreadWithMessageQueueAsync<GitOperationResult>(() =>
		{
			try
			{
                ...
                {
                    Commands.Checkout(repository, checkoutBranch, options);
                }
	...
	public static async Task PullOriginAsync(IFolderViewViewModel folderViewViewModel, string? repositoryPath)
	{
		if (string.IsNullOrWhiteSpace(repositoryPath))
        {
            return;
		...
		{
			await RequireGitAuthenticationAsync(folderViewViewModel);
		}
	...
	public static async Task PushToOriginAsync(IFolderViewViewModel folderViewViewModel, string? repositoryPath, string? branchName)
	{
	...
	public static async Task RequireGitAuthenticationAsync(IFolderViewViewModel folderViewViewModel)
	{
		var pending = true;
	...
	public static GitItemModel GetGitInformationForItem(Repository repository, string path, bool getStatus = true, bool getCommit = true)
	{
		...
		if (getStatus)
		{
			...
			foreach (var c in repository.Diff.Compare<TreeChanges>(repository.Commits.FirstOrDefault()?.Tree, DiffTargets.Index | DiffTargets.WorkingDirectory))
			{
	...
	public static async Task InitializeRepositoryAsync(IFolderViewViewModel folderViewViewModel, string? path)
	{
		if (string.IsNullOrWhiteSpace(path))
        {
            return;
