...
public class FilesystemOperations : IFilesystemOperations
{
	...
	public FilesystemOperations(IFolderViewViewModel folderViewViewModel, IShellPage associatedInstance)
	{
				...
				if (!fsResult)
				{
					...
					if (fsResult)
					{
						...
						if (!fsResultMove) // Use generic move folder operation (move folder items one by one)
						{
							...
							if (await DialogDisplayHelper.ShowDialogAsync(_folderViewViewModel, "ErrorDialogThisActionCannotBeDone".GetLocalizedResource(), "ErrorDialogUnsupportedMoveOperation".GetLocalizedResource(), "OK".GetLocalizedResource(), "Cancel".GetLocalizedResource()))
                            {
		...
		bool asAdmin = false)
	{
		...
		if (!string.IsNullOrWhiteSpace(newName) &&
			!FilesystemHelpers.ContainsRestrictedCharacters(_folderViewViewModel, newName) &&
			!FilesystemHelpers.ContainsRestrictedFileName(newName))
		{
			...
			{
				await DialogDisplayHelper.ShowDialogAsync(_folderViewViewModel, "RenameError/NameInvalid/Title".GetLocalizedResource(), "RenameError/NameInvalid/Text".GetLocalizedResource());
			}
			else if (renamed == FileSystemStatusCode.NameTooLong)
			{
				await DialogDisplayHelper.ShowDialogAsync(_folderViewViewModel, "RenameError/TooLong/Title".GetLocalizedResource(), "RenameError/TooLong/Text".GetLocalizedResource());
			}
			...
			else if (renamed == FileSystemStatusCode.AlreadyExists)
			{
				...
				var result = await ItemAlreadyExistsDialog.TryShowAsync(_folderViewViewModel);
	...
	public async Task<IStorageHistory> RestoreFromTrashAsync(IStorageItemWithPath source, string destination, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken)
	{
		...
		if (!fsResult)
		{
			if (source.ItemType == FilesystemItemType.Directory)
			{
				var sourceFolder = await _associatedInstance.FilesystemViewModel.GetFolderFromPathAsync(source.Path);
				var destinationFolder = await _associatedInstance.FilesystemViewModel.GetFolderFromPathAsync(PathNormalization.GetParentDir(destination));
			...
			{
				var sourceFile = await _associatedInstance.FilesystemViewModel.GetFileFromPathAsync(source.Path);
				var destinationFolder = await _associatedInstance.FilesystemViewModel.GetFolderFromPathAsync(PathNormalization.GetParentDir(destination));
		...
		if (fsResult != FileSystemStatusCode.Success)
		{
			...
			{
				await DialogDisplayHelper.ShowDialogAsync(_folderViewViewModel, "AccessDenied".GetLocalizedResource(), "AccessDeniedDeleteDialog/Text".GetLocalizedResource());
			}
