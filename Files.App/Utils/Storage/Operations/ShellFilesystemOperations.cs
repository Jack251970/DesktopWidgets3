...
public sealed class ShellFilesystemOperations(IFolderViewViewModel folderViewViewModel, IShellPage associatedInstance) : IFilesystemOperations
{
	...
	public async Task<(IStorageHistory, IStorageItem)> CreateAsync(IStorageItemWithPath source, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken, bool asAdmin = false)
	{
		...
		switch (source.ItemType)
		{
			case FilesystemItemType.File:
				{
					...
					{
						(success, response) = await FileOperationsHelpers.CreateItemAsync(source.Path, "CreateFile", _folderViewViewModel.WindowHandle.ToInt64(), asAdmin, newEntryInfo?.Template!, newEntryInfo?.Data);
					}
			...
			case FilesystemItemType.Directory:
				{
					(success, response) = await FileOperationsHelpers.CreateItemAsync(source.Path, "CreateFolder", _folderViewViewModel.WindowHandle.ToInt64(), asAdmin);
					break;
				}
		}
	...
	public async Task<IStorageHistory> CreateShortcutItemsAsync(IList<IStorageItemWithPath> source, IList<string> destination, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken)
	{
		...
		foreach (var item in items)
		{
			...
			if (!result)
            {
                result = await UIFilesystemHelpers.HandleShortcutCannotBeCreated(_folderViewViewModel, Path.GetFileName(item.dest), item.src.Path);
            }
