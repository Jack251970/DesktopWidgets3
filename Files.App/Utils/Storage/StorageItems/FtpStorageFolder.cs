...
public sealed class FtpStorageFolder : BaseStorageFolder, IPasswordProtectedItem
{
	...
	public static FtpStorageFolder CloneWithPath(string path) => new(new StorageFolderWithPath(null!, path));
	...
	public override IAsyncOperation<BaseStorageFolder> CreateFolderAsync(string desiredName, CreationCollisionOption options)
	{
		return AsyncInfo.Run((cancellationToken) => SafetyExtensions.Wrap<BaseStorageFolder>(async () =>
		{
			...
			var fileName = $"{FtpPath}/{desiredName}";
			...
			var replaceExisting = options is CreationCollisionOption.ReplaceExisting;
			var isSuccessful = await ftpClient.CreateDirectory(fileName, replaceExisting, cancellationToken);
			if (!isSuccessful)
			{
	...
	public override IAsyncOperation<BaseStorageFolder> MoveAsync(IStorageFolder destinationFolder, NameCollisionOption option)
	{
		return AsyncInfo.Run((cancellationToken) => SafetyExtensions.Wrap<BaseStorageFolder>(async () =>
		{
			...
			{
				var destName = $"{ftpFolder.FtpPath}/{Name}";
				var ftpRemoteExists = option is NameCollisionOption.ReplaceExisting ? FtpRemoteExists.Overwrite : FtpRemoteExists.Skip;

				var isSuccessful = await ftpClient.MoveDirectory(FtpPath, destName, ftpRemoteExists, token: cancellationToken);
				if (!isSuccessful)
                {
	...
	public override IAsyncAction RenameAsync(string desiredName, NameCollisionOption option)
	{
		return AsyncInfo.Run((cancellationToken) => SafetyExtensions.WrapAsync(async () =>
		{
			...
			var destination = $"{PathNormalization.GetParentDir(FtpPath)}/{desiredName}";
