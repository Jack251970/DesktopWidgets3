...
public abstract class BaseShellPage : Page, IShellPage, INotifyPropertyChanged
{
    ...
    public ShellViewModel ShellViewModel { get; protected set; } = null!;
	...
	protected IShellPanesPage _PaneHolder = null!;
	...
	protected TabBarItemParameter _TabItemArguments = null!;
	...
	{
		if (!await GitHelpers.Checkout(FolderViewViewModel, ShellViewModel.GitDirectory, branchName))
		{
	...
	public async Task Refresh_Click()
	{
		if (InstanceViewModel.IsPageTypeSearchResults)
		{
            ...
            await ShellViewModel.SearchAsync(FolderViewViewModel, searchInstance);
        }
	...
	{
		foreach (var entry in ItemDisplay.BackStack.ToList())
		{
				...
				args.NavPathParam is not null and not "Home")
			{
				...
				{
					var index = ItemDisplay.BackStack.IndexOf(entry);
		...
		foreach (var entry in ItemDisplay.ForwardStack.ToList())
		{
			if (entry.Parameter is NavigationArguments args &&
				args.NavPathParam is not null and not "Home")
			{
				...
				if (!entry.SourcePageType.Equals(correctPageType))
				{
					var index = ItemDisplay.ForwardStack.IndexOf(entry);
					var newEntry = new PageStackEntry(correctPageType, entry.Parameter, entry.NavigationTransitionInfo);
