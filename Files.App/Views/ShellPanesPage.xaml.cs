...
public sealed partial class ShellPanesPage : Page, IShellPanesPage, ITabBarItemContent
{
    ...
    private AppModel AppModel { get; } = DependencyExtensions.GetRequiredService<AppModel>();
    ...
    public bool IsMultiPaneEnabled
    {
        get
        {
            try
            {
                return !AppModel.IsMainWindowClosed && FolderViewViewModel.Bounds.Width > Constants.UI.MultiplePaneWidthThreshold;
            }
            catch
            {
                return false;
    ...
    // CHANGE: Use non-static event handler to support multiple instances.
    public event EventHandler<ShellPanesPage>? CurrentInstanceChanged;
    ...
    public ShellPanesPage()
    {
        ...
        // Set default values
        ActivePane = GetPane(0);
        /*_WindowIsCompact = FolderViewViewModel.Bounds.Width <= Constants.UI.MultiplePaneWidthThreshold;
        IsRightPaneVisible = IsMultiPaneEnabled && UserSettingsService.GeneralSettingsService.AlwaysOpenDualPaneInNewTab;

        FolderViewViewModel.MainWindow.SizeChanged += MainWindow_SizeChanged;*/
    }

    // CHANGE: Initialize folder view view model and related services.
    private void Initialize(NavigationEventArgs eventArgs)
    {
        // TODO: Check if need.
        ...
        if (FolderViewViewModel is null)
        {
            ...
            var pageContext = FolderViewViewModel.GetRequiredService<IMultiPanesContext>();
    ...
    {
        WindowIsCompact = FolderViewViewModel.Bounds.Width <= Constants.UI.MultiplePaneWidthThreshold;
    }
