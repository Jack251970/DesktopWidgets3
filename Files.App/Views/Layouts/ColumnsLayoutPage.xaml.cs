...
public sealed partial class ColumnsLayoutPage : BaseLayoutPage
{
	...
	protected override void OnNavigatedTo(NavigationEventArgs eventArgs)
	{
		...
		{
			var rootPathList = DependencyExtensions.GetService<QuickAccessManager>().Model.FavoriteItems.Select(NormalizePath)
				.Concat(CloudDrivesManager.Drives.Select(x => NormalizePath(x.Path))).ToList()
				.Concat(DependencyExtensions.GetService<LibraryManager>().Libraries.Select(x => NormalizePath(x.Path))).ToList();
	...
	{
		CommandsViewModel = new BaseLayoutViewModel(FolderViewViewModel, ParentShellPageInstance!, ItemManipulationModel);
	}
		...
		if (index >= 0)
		{
			SafetyExtensions.IgnoreExceptions(() =>
			{
				while (ColumnHost.ActiveBlades.Count > index + 1)
				{
                    ...
                    if (frame?.Content is ColumnShellPage { SlimContentPage: ColumnLayoutPage columnLayout })
					{
	...
	private static ColumnLayoutPage? RetrieveBladeColumnViewBase(BladeItem blade)
	{
	...
	public void SetSelectedPathOrNavigate(string navigationPath, Type sourcePageType, NavigationArguments navArgs = null!)
	{
		...
		else
		{
			...
			for (var ii = lastCommonItemIndex + 1; ii < destComponents.Count; ii++)
			{
				var (frame, newblade) = CreateAndAddNewBlade();
