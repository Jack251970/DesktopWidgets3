...
public sealed partial class ColumnLayoutPage : BaseGroupableLayoutPage
{
	...
	protected override void FileList_SelectionChanged(object sender, SelectionChangedEventArgs e)
	{
        ...
        if (SelectedItems?.Count == 1 && SelectedItem?.PrimaryItemAttribute is StorageItemTypes.Folder)
		{
            ...
            {
                ItemInvoked?.Invoke(new ColumnParam { FolderViewViewModel = FolderViewViewModel, Source = this, NavPathParam = (SelectedItem is ShortcutItem sht ? sht.TargetPath : SelectedItem.ItemPath), ListView = FileList }, EventArgs.Empty);
            }
            else
            {
                CloseFolder();
		...
		if (clickedItem?.DataContext is ListedItem item)
		{
			switch (item.PrimaryItemAttribute)
			{
				...
				case StorageItemTypes.Folder:
                    ...
                    {
                        ItemInvoked?.Invoke(new ColumnParam { FolderViewViewModel = FolderViewViewModel, Source = this, NavPathParam = (item is ShortcutItem sht ? sht.TargetPath : item.ItemPath), ListView = FileList }, EventArgs.Empty);
                    }
	...
	private void CheckDoubleClick(ListedItem item)
	{
		doubleClickTimer.Debounce(() =>
		{
			...
			ItemTapped?.Invoke(new ColumnParam { FolderViewViewModel = FolderViewViewModel, Source = this, NavPathParam = Path.GetDirectoryName(itemPath), ListView = FileList }, EventArgs.Empty);
