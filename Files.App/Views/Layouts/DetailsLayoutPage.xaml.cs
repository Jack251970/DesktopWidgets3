...
public sealed partial class DetailsLayoutPage : BaseGroupableLayoutPage
{
	...
	public override void StartRenameItem()
	{
        ...
        if (listViewItem.FindDescendant("ItemNameTextBox") is not TextBox textBox || textBox.FindParent<Grid>() is null)
        {
            return;
        }
	...
	private void SizeAllColumnsToFit_Click(object sender, RoutedEventArgs e)
	{
        ...
        var totalColumnCount = ColumnsViewModel.GetType().GetProperties().Count(prop => prop.PropertyType == typeof(DetailsLayoutColumnItem));
		for (var columnIndex = 1; columnIndex <= totalColumnCount; columnIndex++)
        {
	...
	private static bool IsCorrectColumn(FrameworkElement element, int columnIndex)
	{
		var columnIndexFromName = element.Name switch
		{
			"ItemName" => 2,
