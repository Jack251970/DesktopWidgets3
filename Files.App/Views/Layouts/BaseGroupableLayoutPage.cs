...
public abstract class BaseGroupableLayoutPage : BaseLayoutPage
{
	...
	protected override void Page_CharacterReceived(UIElement sender, CharacterReceivedRoutedEventArgs args)
	{
		if (ParentShellPageInstance is null ||
			ParentShellPageInstance.CurrentPageType != GetType() ||
			IsRenamingItem)
        {
            return;
        }
	...
	protected async virtual void ItemManipulationModel_RefreshItemsThumbnail(object? sender, EventArgs e)
	{
		await ReloadSelectedItemsIconAsync();
	}

	protected async virtual void ItemManipulationModel_RefreshItemThumbnail(object? sender, EventArgs args)
	{
		await ReloadSelectedItemIconAsync();
	}

	protected async virtual Task ReloadSelectedItemIconAsync()
	{
	...
	protected async virtual Task ReloadSelectedItemsIconAsync()
	{
	...
	protected virtual void ItemManipulationModel_InvertSelectionInvoked(object? sender, EventArgs e)
	{
		...
		var newSelectedItems = GetAllItems()!
			.Cast<ListedItem>()
