...
public class CachedSizeProvider : ISizeProvider
{
	...
	public async Task UpdateAsync(string path, CancellationToken cancellationToken)
	{
		...
		async Task<ulong> Calculate(string path, int level = 0)
		{
			...
			var hFile = FindFirstFileExFromApp($"{path}{Path.DirectorySeparatorChar}*.*", FINDEX_INFO_LEVELS.FindExInfoBasic,
				out var findData, FINDEX_SEARCH_OPS.FindExSearchNameMatch, IntPtr.Zero, FIND_FIRST_EX_LARGE_FETCH);
