...
public abstract class BasePreviewModel : ObservableObject
{
    ...
    public List<FileProperty> DetailsFromPreview
    {
        get; set;
    }
    ...
    {
        var dispatcherQueue = DesktopWidgets3.App.DispatcherQueue;
        ...
        {
            await dispatcherQueue.EnqueueOrInvokeAsync(async () => FileImage = (await iconData.ToBitmapAsync())!);
        }
        else
        {
            FileImage ??= await dispatcherQueue.EnqueueOrInvokeAsync(() => new BitmapImage());
        }
