...
public class NetworkDrivesAPI
{
    ...
    {
        private readonly NETRESOURCE nres = new();
        ...
        {
            get => nres.lpRemoteName; set => nres.lpRemoteName = value;
        }
    ...
    public static async Task<bool> AuthenticateNetworkShare(FolderViewViewModel viewModel, string path)
    {
        ...
        var connectionError = WNetAddConnection3(HWND.NULL, nr, null, null, 0);  // if creds are saved, this will return NO_ERROR
        ...
        if (connectionError == Win32Error.ERROR_LOGON_FAILURE)
        {
            ...
            if (credentialsReturned is not null && credentialsReturned[1] != null)
            {
                ...
                if (credentialsReturned[2] == "y" && connectionError == Win32Error.NO_ERROR)
                {
                    ...
                    {
                        TargetName = new StrPtrAuto(path[2..]),
        ...
        {
            await DialogDisplayHelper.ShowDialogAsync(viewModel, "NetworkFolderErrorDialogTitle".GetLocalized(), connectionError.ToString().Split(":")[1].Trim());
            return false;
        }
    }
