...
{
    private StorageItemQueryResult StorageItemQueryResult
    {
        get;
    }
    ...
    public override IAsyncOperation<IReadOnlyList<IStorageItem>> GetItemsAsync(uint startIndex, uint maxNumberOfItems)
    {
        return AsyncInfo.Run<IReadOnlyList<IStorageItem>>(async (cancellationToken) =>
        {
            ...
            return items.Select(x => x is StorageFolder ? (IStorageItem)new SystemStorageFolder((StorageFolder)x) : new SystemStorageFile((StorageFile)x)).ToList();
        });
    }
...
{
    private StorageFileQueryResult StorageFileQueryResult
    {
        get;
    }
...
{
    private StorageFolderQueryResult StorageFolderQueryResult
    {
        get;
    }
