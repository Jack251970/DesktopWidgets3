...
public static class DriveHelpers
{
    ...
    {
        var name = driveName.ToUpperInvariant();
        var query = $"SELECT DeviceID FROM Win32_Volume WHERE DriveLetter = '{name}'";
    ...
    public static Data.Items.DriveType GetDriveType(DriveInfo drive)
    {
        if (drive.DriveType is System.IO.DriveType.Unknown)
        {
            var path = PathNormalization.NormalizePath(drive.Name);
    ...
    public static string GetExtendedDriveLabel(DriveInfo drive)
    {
        ...
        {
            return drive.DriveType is not System.IO.DriveType.CDRom || drive.DriveFormat is not "UDF"
                ? drive.VolumeLabel
